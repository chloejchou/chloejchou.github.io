window.addEventListener('DOMContentLoaded', (event) => {
  ...
  const arrowDown = document.querySelector('.feather-arrow-down')
  ...
  const step = () => {
    const quoteContainer = document.querySelector('#large-quote');
    const bounding = quoteContainer.getBoundingClientRect();
    const isInView = bounding.top > 0 || bounding.bottom > 0;
    if (!isInView) {
